...
<span
    ...
    :title="`{{ strtoupper($locale) }}: ` + (translations[{{ json_encode($locale) }}] && translations[{{ json_encode($locale) }}].trim() ? {{ json_encode($translatedLabel) }} : {{ json_encode($missingLabel) }})"
...
>
    <span
        :class="translations[{{ json_encode($locale) }}] && translations[{{ json_encode($locale) }}].trim() ? 'bg-success-500' : 'bg-danger-300 dark:bg-danger-600'"
